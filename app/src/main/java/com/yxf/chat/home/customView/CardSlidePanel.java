...
public class CardSlidePanel extends ViewGroup {
    private List<CardItemView> viewList = new ArrayList<CardItemView>(); // 存放的是每一层卡片的view，从顶到底
    private List<View> releasedViewList = new ArrayList<View>(); // 手指松开后存放的view集合,即已释放的卡片View集合
    ...
    private int yOffsetStep = 5; // view叠加垂直偏移量的步长
    private int mTouchSlop = 5; // 判定为滑动的最小值，单位是像素
    ...
    private List<CardDataBean> dataList; // 存储的数据链表
...
//        bottomMarginTop = (int) a.getDimension(R.styleable.card_bottomMarginTop, bottomMarginTop);
        ...
        btnListener = new OnClickListener() {
            ...
            public void onClick(View view) {
                if (view.getId() == R.id.card_banner) {
                    ...
                    // 点击的是bottomLayout里面的一些按钮,即喜欢和不喜欢的按钮
                    btnLock = true;
                    int type = -1;
                    if (view == leftBtn) {
    ...
    protected void onFinishInflate() {
        ...
        for (int i = num - 1; i >= 0; i--) {
            ...
            if (childView.getId() == R.id.card_bottom_layout) {
                ...
                // for循环取view的时候，是从外层往里取卡片,并放入对应集合viewList
    ...
    //提供底部按钮操作方法给外面调用
    public void setLeftBottomButtonChecked() {
        leftBtn.setPressed(true);
    }

    public void setRightBottomButtonChecked() {
        rightBtn.setPressed(true);
    }

    public void setBottomButtonUnchecked() {
        leftBtn.setPressed(false);
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            ...
            if (child == bottomLayout || dataList == null || dataList.size() == 0
                    || child.getVisibility() != View.VISIBLE || child.getScaleX() <= 1.0f -
                    SCALE_STEP) {
    ...
    private void orderViewStack() {
        ...
        if (newIndex < dataList.size()) {
            CardDataBean dataItem = dataList.get(newIndex);
    ...
    private void vanishOnBtnClick(int type) {
        ...
        if (finalX != 0) {
            ...
            //卡片移动的关键代码动画移动view到指定位置,有个过程,返回false说明动画结束
            //返回true需要继续调用continueSettling直到返回false
    ...
    public void computeScroll() {
        //continueSettling返回true说明还在进度中,与Scroller机制类似
        if (mDragHelper.continueSettling(true)) {
            ViewCompat.postInvalidateOnAnimation(this);
        } else {
            // 获取当前拖动状态,STATE_IDLE参数表示拖动结束,即动画结束
...
//                maxHeight);
     ...
     */
    public void fillData(List<CardDataBean> dataList) {
